...
def spotify_auth() -> str:
    ...
    client_id: str = apikeys.client_id
    client_secret: str = apikeys.client_secret
...
def make_search_names(tracks: list[dict]) -> list[str]:
    ...
    # print(tracks)
    # for actual, track in enumerate(tracks):
    #     print(f'track n{actual}: {track["track"]["artists"][0]["name"]} - {track["track"]["name"]}')
    search_names = []
    for track in tracks:
        if track["track"] is not None:
            search_names.append(f'{track["track"]["artists"][0]["name"]} - {track["track"]["name"]}')
        else:
